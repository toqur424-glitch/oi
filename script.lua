...
-- =========================================================================
-- XOCU 스타일 루프 그랩 킥 (tp back) - 토글 추가
-- =========================================================================
local kickTpBackActive = false
local kickTpBackConn = nil
local kickTpBackSavedPos = nil
local kickTpBackStartTime = 0

local function startTpBackLoop()
    if kickTpBackConn then kickTpBackConn:Disconnect() end

    kickTpBackActive = true
    kickTpBackStartTime = 0
    kickTpBackSavedPos = nil

    kickTpBackConn = RunService.Heartbeat:Connect(function()
        if not kickTpBackActive or not selectedKickPlayer then return end
        ...
        if not (myChar and myHRP and myHead and tHRP and tHum and tHum.Health > 0) then
            return
        end

        local dist = (tHRP.Position - myHRP.Position).Magnitude

        -- FETCH: 거리가 30 이상이면 자신을 대상 위치로 텔레포트
        if dist > 30 and not kickTpBackSavedPos then
            pcall(function()
                myChar:PivotTo(tHRP.CFrame * CFrame.new(0, 2, 4))
            end)
            pcall(function()
        ...
        -- 현재 위치 저장 (복귀 지점)
        if not kickTpBackSavedPos and dist <= 30 then
            kickTpBackSavedPos = myHRP.CFrame
            kickTpBackStartTime = tick()
        end

        -- AlignPosition이 없으면 생성 (기존 함수 재사용)
        if not tHRP:FindFirstChild("KickAlign") then
            setupAlignForTarget()
        end
        ...
        -- 0.3초 후 원래 위치로 함께 복귀 (XOCU 원본 동작)
        if kickTpBackSavedPos and tick() - kickTpBackStartTime > 0.3 then
            pcall(function()
                myChar:PivotTo(kickTpBackSavedPos)
                tHRP.CFrame = kickTpBackSavedPos * CFrame.new(0, 20, 0)
            end)
            kickTpBackSavedPos = nil
            kickTpBackStartTime = 0
        end
...
local function stopTpBackLoop()
    kickTpBackActive = false
    if kickTpBackConn then
        kickTpBackConn:Disconnect()
        kickTpBackConn = nil
    end
    kickTpBackSavedPos = nil
    kickTpBackStartTime = 0
...
KickTab:CreateToggle({
    Name = "XOCU 루프 킥 (tp back) - 자동 복귀",
    Flag = "TpBackLoopToggle",
    Default = false,
    Callback = function(v)
        if v and not selectedKickPlayer then
            Rayfield:Notify({Title = "알림", Content = "먼저 타겟 닉네임을 입력해주세요!", Duration = 3})
...
Rayfield:Notify({Title = "로딩 완료", Content = "안티그랩 유지, X=7, Y=20, 350Hz 정밀 타이머 + XOCU tp back", Duration = 3})
