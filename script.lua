...
GrabTab:CreateSection("=== 킥 그랩 (SetNetworkOwner×3 → SetNetworkOwner+DestroyGrabLine 동시×2 / 600틱) ===")
...
local function startFKeyAttack(targetPlayer)
    ...
    -- ✅ 패턴: SetNetworkOwner ×3 → (SetNetworkOwner + DestroyGrabLine 동시) ×2
    --  1 = SetNetworkOwner only
    --  3 = SetNetworkOwner + DestroyGrabLine 동시
    local PATTERN = {1, 1, 1, 3, 3}
    local patternIdx = 1
    local TICK_HZ = 600
    local tickAccum = 0
...
GrabTab:CreateToggle({
    Name = "카메라 조준 킥 그랩 실행 (SetNetworkOwner×3 → SetNetworkOwner+DestroyGrabLine 동시×2 / 600틱)",
    Callback = function(v)
...
KickTab:CreateToggle({
    Name = "블롭맨 오너 킥 실행 (SetNetworkOwner×3 → SetNetworkOwner+DestroyGrabLine 동시×2 / 600틱 / 전신 박제)",
    Callback = function(v)
...
Rayfield:Notify({Title = "로딩 완료", Content = "패턴 SetNetworkOwner×3 → (SetNetworkOwner+DestroyGrabLine 동시)×2 / 600틱 / 몸통 정밀", Duration = 3})
